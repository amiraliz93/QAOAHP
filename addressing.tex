...
The pipeline is defined using the variables \(\phi, c, s, \xi, A\). Meanings are,
\begin{itemize}
...
\begin{equation}
    ...
    \end{matrix} 
\end{equation}
where \(\hatj\) is an index defined in section \ref{sAddrGen}. If \(q\) is successor index supplied to the pipeline and \(\xi_1' = 00\), it must be satisfied that \((\hatj, q)\) is a pair indexes of the amplitudes to be mixed. That is, for some \(n\in[N]\), \((\hatj, q)\in S_n\). If the input is end of the pair, \(\xi_1'\) must be 01. The way above supplying \(\xi_1' = 0j_0\) will satisfies these restriction.

The meaning of precedent \(0\) for \(\xi\) is explained in the reuse method of this unit for cost function operation later. First this document describes the use of this pipeline in mixer operation.
...
Assuming we got \(\psi\in \sC^D\) as an initial state vector and got \(\psi^L\in\sC^D\) by applying mixer operation to the local qbits (qbit such that its number \(n\) satisfies \(n\leq \hatM\).). Superscrpit \(L\) means the vector applied \(L\) bits local mixer operation. Then, let \(\psi^{\sigma L}\) be a state vector that shuffled its amplitude position as,
\begin{equation}
    \label{eshuffle}
    \psi^{\sigma L}_\jbar = \psi_{L\sigma(\jbar)} , \Leftrightarrow\psi^{\sigma L}_{p_N\cdots p_1}= \psi^L_{p_{N-\hatM}\cdots p_1 p_N\cdots p_{N-\hatM+1}}
\end{equation}
