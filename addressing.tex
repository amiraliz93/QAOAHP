...
In this document \(\hatM\)-th greatest bits are called global qbits. Remaining \((N-\hatM)\) bits are called local qbits. Size of space that global qbits compose can be greater than number of parallel blocks, thus, \(K = 2^M \leq 2^\hatM\).
...
\jbar = k\hatj,
\]
mixer operations are applied to \(n\in[N-\hatM]\) qbit for each \(k\)-th block (\(\hatj\) is defined in the same way as in section \ref{sAddrGen}). Note that only the part of \(\hatj\) above is used to access BRAMs. For intuitively, consider the case \(k = 5=101, j=11001\), then this notation means \(\jbar = 10111001\). 
...
Assuming we got \(\psi\in \sC^D\) as an initial state vector and got \(\hpsi\in\sC^D\) by applying mixer operation to the local qbits (qbit such that its number \(n\) satisfies \(n\leq \hatM\).). Then, let \(\hpsi'\) be a state vector that shuffled its amplitude position as,
\begin{equation}
    \label{eshuffle}
    \hpsi'_\jbar = \hpsi_{\sigma(\jbar)} , \Leftrightarrow\hpsi'_{p_N\cdots p_1}= \hpsi_{p_{N-\hatM}\cdots p_1 p_N\cdots p_{N-\hatM+1}}
\end{equation}
...
\]
\textit{Global mixer}: 
Then we apply mixer operation on the qbits of \(n \in[\hatM]\) to \(\hpsi'\) and we obtain \(\psi^@\). It can be done as completely same as in the case of local mixer. In detail, 
...
\textit{Restoration}: Let \(\psi'\in\sC^D\) be a final result of original order, it can be obtained,
\begin{equation}
    \label{ereverseshuffle}
\psi'_\jbar = \psi^@_{\sigma^{-1}(\jbar)}, \Leftrightarrow \psi'_{p_N\cdots p_1}= \psi^@_{p_{\hatM}\cdots p_1 p_N\cdots p_{\hatM+1}}.
\end{equation}
...
Proof:  \( U_n \psi\) is described  for all index \(p = 0,1,...,{D-1}, q, (p, q)\in S_n\) (such \(q\) exists uniquely) as,
\begin{align*}
...
\begin{align*}
    ...
    \br{\Theta_{n, 1}h}_{q} &= h_{b}, q_N\cdots q_1 = q_N\cdots b_{n+1}b_1 b_{n-1}\cdots b_2 b_n.
\end{align*}
...
\end{align*}
If we substitute \(h = \Theta_{n,1}\psi\) and applying \( \Theta_{n,1}\Theta_{n,1}\psi = \psi\), then, for all \(\psi\in\sC^D\),ßß
\begin{equation}
...
\end{equation}
Proof:  \( U_l \psi\) is described as for all index \(u = 0,1,...,{D-1}, q, (u, v)\in S_l\) (such \(v\) exists uniquely),
\begin{align*}
    \br{U_l \psi}_u &= \psi_u \cos\beta - \ci\psi_v \sin \beta.
\end{align*}
When applying \(\Theta_{n, 1}, l\not\in\cbr{n, 1}\), there exist indexes \(c, d=0,1,...,D-1\) that satisfy for arbitrary \(h\in\sC^D\), 
\begin{align*}
    \br{\Theta_{n, 1}h}_{u} &= h_{c}, u_N\cdots u_1 = c_N\cdots c_{n+1}c_1 c_{n-1}\cdots c_2 c_n,\\
    \br{\Theta_{n, 1}h}_{v} &= h_{d}, v_N\cdots v_1 = d_N\cdots d_{n+1}d_1 d_{n-1}\cdots d_2 d_n.
\end{align*}
Now we know \(u_n = v_n, u_1=v_1\) from definition and \(l\not\in \cbr{n, 1}\). So \(c_l = u_l, d_l= v_l, u_l\neq v_l\), so \(c_l\neq d_l\) holds. For another case, \(\forall i\in N_l,  c_i=d_i\). It means \((c, d)\in S_l\) too. Therefore,
\begin{align*}
...
\begin{equation}
   \br{\Theta_{n,1} U_l \Theta_{n,1}\psi }_{u}  = \br{\psi}_u\cos\beta - \ci\br{\psi}_v \sin \beta = \br{U_l \psi}_u.
\end{equation}
...
\begin{equation}
    \label{erotateB}
    U_{N-n} \psi = R_{-n}  U_{1} R_{n}\psi,
\end{equation}
where,
\begin{align*}
R_{n}\psi_{p_N\cdots p_1} &= \psi_{p_{N-n}\cdots p_1p_N\cdots p_{N-n+1}},\\
R_{-n}\psi_{p_N\cdots p_1}&= \psi_{p_{n}\cdots p_1p_N\cdots p_{n+1}},\\
R_n = \Theta_{n, m}
\end{align*}
...
\[
    U_{N}U_{N-1} \psi = R_{-n}  U_{2}  R_{n}R_{-n}U_{1} R_n\psi =R_{-n}  U_{2} U_{1} R_n\psi 
\]
