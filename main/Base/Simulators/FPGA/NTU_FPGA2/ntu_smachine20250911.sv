...
reg [63:0] res_addFP64;
reg [63:0] res_mulFP64;
addFPF64 addFPF64(
      .clk(CLK),    //    clk.clk
...
parameter OP_SEND_CMD = 115;  // Send: 0, Res: 0.
...
parameter FETCH_w_BRAM = 6;

wire [3:0] wstoreReg = tf_full? STORE_WAIT: storeReg;
wire [3:0] wfetchReg = rf_dv[0]? fetchReg: FETCH_WAIT;

reg [4:0] writeReg;
reg [4:0] bwriteReg;
logic [4:0] n_writeReg;
logic [4:0] nb_writeReg;

parameter WRITE_NONE = 0;
parameter WRITE_T2A = 1;
parameter WRITE_T2B = 2;
parameter WRITE_A2B = 3;
parameter WRITE_A2U = 4;
parameter WRITE_B2A = 5;
parameter WRITE_mulFP64_rA = 6;
parameter WRITE_addFP64_rA = 7;
parameter WRITE_mul_rA = 8;
parameter WRITE_add_rA = 9;
parameter WRITE_rA1 = 10;
parameter WRITE_rB1 = 11;
parameter WRITE_BRAM_U = 12;
...
always_comb begin: main_StateBlock
      ...
      s_Operation: begin
            ...
            OP_READ_RAM2U: begin
                  ...
                  // 2025 September, no streaming supported
                  n_r_req = 1;
                  n_state = s_READ_BRAM;
            end
            default: begin // idle
      ...
      s_READ_BRAM:begin
            n_r_req = 0; // 2025 September, no streaming supported, read one by one. Do not mind latency. 1 data per 4 cycle.
            if(rbram_vd) begin
      ...
      s_WRITE_BRAM: begin
            n_w_req = 0; // only one word data will be written.
            n_state = s_IDLE;
      end
      s_WAIT_COMP: begin 
            n_c_wait = c_wait + 1;
