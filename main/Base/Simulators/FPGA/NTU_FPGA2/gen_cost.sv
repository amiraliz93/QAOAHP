...
localparam N0 = 12; // latency new test_mul 12 cycles
localparam N1 = 172; // latency of CORDIC. 56.53(sign) to 55.53(sign)

reg signed [P-1:0] mul_in1;
reg signed [P-1:0] mul_in2;
reg signed [2*P-1:0] slcier_in;
...
CORDIC_64_fixedP inst_cordic(
		...
		.c(cordic_cos_out),         // c.c output wire [62:0] , cosine
		.clk(CLK),       // clk.clk         
		.s(cordic_sin_out)          // s.s     output wire [62:0] , sine
	);
